...
{
    has @body-attributes = ['client-oid',
			    'type',
			    'side',
			    'product-id',
			    'stp',
    ...
    has OrderTimeInForce $.time-in-force is rw;
    has                  $.cancel-after  is rw;
    ...
    ## Stop orders
    #
    has Bool        $.overdraft-enabled is rw;
    has PositiveNum $.funding-amount    is rw;

    method initiate() {
	fail 'Orders need side, "buy" or "sell"' unless $.side;
	fail 'Orders need a product-id'          unless $.product-id;
	if ($.type eq 'limit') {
	    fail 'Limit orders need price' unless $.price;
	    fail 'Limit orders need size'  unless $.size;
	    if ($.cancel-after) {
		unless ($.time-in-force eq 'GTT') {
		    fail 'time-in-force must be "GTT" when using cancel-after';
		}
	    }
	    if ($.post-only) {
		if ($.time-in-force eq any <IOC FOK>) {
		    fail 'post-only is invalid with time-in-force IOC or FOK';
		}
	    }
	}
...
The optional client-oid field must be a UUID generated by your trading
...
The client-oid is different than the server-assigned order id. If you
are consuming the public feed and see a received message with your
client-oid, you should record the server-assigned order_id as it will
be used for future order status updates. The client-oid will NOT be
...
and not the client-selected client-oid.
